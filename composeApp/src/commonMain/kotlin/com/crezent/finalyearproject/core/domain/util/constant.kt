package com.crezent.finalyearproject.core.domain.util

object Constant {
}

object ApiRoute {
    const val BASE_URL = "http://172.20.10.12:8080/"
    // const val BASE_URL = "http://192.168.81.175:8080" // samsung
    //172.20.10.12
    // const val SAMSUNG_BASE_URL = "http://192.168.81.175:8080"

    //192.168.113.175
    // const val BASE_URL = "https://wallet-server-9q2n.onrender.com/" // RENDER


    const val LOGIN = "login"
    const val SIGNUP = "sign-up"
    const val API_KEY = "public-key"
    const val REQUEST_OTP = "request-otp"
    const val RESET_PASSWORD = "reset-password"
    const val VERIFY_MAIL = "verify-mail"
    const val VERIFY_OTP = "verify-otp"
    const val VERIFY_CVV = "verify-cvv"
    const val AUTHENTICATED_USER = "authenticated-user"
    const val CREATE_CARD = "create-card"
    const val INITIATE_TRANSACTION = "initiate-transaction"
    const val VERIFY_DEPOSIT = "verify-deposit"

}

object Animations {
    const val ACCEPTED_RELAX = "files/animation/accepted_relax.json"
    const val ERROR = "files/animation/error.json"
    const val LOADING_CIRCLE = "files/animation/loading_circle.json"
    const val LOADING_HORIZONTAL = "files/animation/loading_horizontal.json"
    const val LOADING_REPLACE = "files/animation/loading_replace.json"
    const val MAIL_SENT = "files/animation/mail_sent.json"
    const val SUCCESSFUL = "files/animation/successful.json"

}

